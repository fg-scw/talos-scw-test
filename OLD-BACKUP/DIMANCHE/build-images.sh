...
ENDSSH
    ...
    if [ $? -ne 0 ]; then
        scw instance server terminate $instance_id zone=$ZONE with-ip=true with-block=true 2>/dev/null || true
    ...
    scw instance server detach-volume volume-id=$volume_id zone=$ZONE 2>/dev/null || true
    sleep 5
    
    # Create snapshot (USING BLOCK API)
    log "Creating BLOCK snapshot from volume..."
    ...
    scw instance server terminate $instance_id zone=$ZONE with-ip=true with-block=true 2>/dev/null || true
    scw block volume delete $volume_id zone=$ZONE 2>/dev/null || true

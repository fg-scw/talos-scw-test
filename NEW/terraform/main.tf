...
output "mig_config" {
  ...
  value = {
    enabled       = var.enable_gpu_mig
    profile       = var.gpu_mig_profile
    expected_gpus = var.gpu_mig_profile == "all-1g.10gb" ? 7 : var.gpu_mig_profile == "all-2g.20gb" ? 3 : var.gpu_mig_profile == "all-3g.40gb" ? 2 : var.gpu_mig_profile == "mixed-40-20" ? 3 : 1
  }
}
